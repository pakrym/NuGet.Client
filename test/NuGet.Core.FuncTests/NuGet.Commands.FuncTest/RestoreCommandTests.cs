...
";
        ...
        public async Task RestoreCommand_RestoreFloatingVersionWithIgnoreFailingLocalSource()
        {
            ...
            using (var projectDir = TestFileSystemUtility.CreateRandomTestFolder())
            {
                ...
                var logger = new TestLogger();
                var context = new cacheSourceContext();
                context.IgnoreFailedSource = true;
                var provider = RestoreCommandProviders.Create(packagesDir,sources,new SourceCacheContext(),logger)
